    ...
    var body: some View {
        VStack {
            ...
            List(viewModel.books.items, id: \.id) { book in
                BookCell(title: book.volumeInfo.title, subtitle: book.volumeInfo.subtitle ?? "", imageUrlString: viewModel.convertToHttps(book.volumeInfo.imageLinks.smallThumbnail), authors: book.volumeInfo.authors, textSnippet: book.searchInfo.textSnippet)
